...
/**
 * 生成包含依赖包和过滤过的资源文件的zip文件
 * @author
 *
 */
@Mojo(name = "zip", inheritByDefault = true, requiresDependencyResolution = ResolutionScope.COMPILE, threadSafe = true)
public class DependZipMojo extends AbstractMojo implements
	 ...
	 */
	@Parameter(defaultValue = "temp", required = true, readonly=true)
	 ...
	 * jar, in addition to other options. See <a
	 ...
	 * @since 2.2-beta-1
	 */
	@Parameter(property = "assembly.attach", defaultValue = "false")
	...
	public void execute() throws MojoExecutionException, MojoFailureException {
		...
		this.descriptorRefs=new String[]{"zip"};
//		if(this.descriptorRefs==null||this.descriptorRefs.length==0){
//		}
		if (skipAssembly) {
			getLog().info(
					"Assemblies have been skipped per configuration of the skipAssembly parameter.");
			return;
		}
				...
				for (final String format : effectiveFormats) {
					...
					if (attach && destFile.isFile()) {
						if (isAssemblyIdAppended()) {
							...
							log.info("destFile1:"+destFile.getAbsolutePath());
							project.getArtifact().setFile(destFile);
						} else {
							log.info("destFile2:"+destFile.getAbsolutePath());
							projectHelper.attachArtifact(project, format, null,
									destFile);
						}
	...
	private FixedStringSearchInterpolator createRepositoryInterpolator() {
		...
		if (getLocalRepository() != null) {
			...
			settingsProperties.setProperty("localRepository", session
	...
	public static FixedStringSearchInterpolator mainProjectInterpolator(
		...
		if (mainProject != null) {
			// 5
			return FixedStringSearchInterpolator
					.create(new org.codehaus.plexus.interpolation.fixed.PrefixedObjectValueSource(
